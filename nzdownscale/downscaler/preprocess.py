...
class PreprocessForDownscaling:
    ...
    def run_processing_sequence(
        ...
        highres_aux_raw_ds, aux_raw_ds = self.preprocess_topography(
    ...
    def load_topography(self):
        ...
        if self.area is not None:
            ...
            self.ds_elev = self.ds_elev.sel(
                latitude=slice(minlat, maxlat), longitude=slice(minlon, maxlon)
            )
    ...
    def load_stations(self, use_cache=False):
        ...
        if use_cache:
            if "cache" not in DATA_PATHS.keys():
                raise ValueError(
                    "Please set 'cache' path in DATA_PATHS dict e.g. "
                    "'cache':'data/.datacache' (recommended) or set use_cache=False "
                )
            savedir = f'{DATA_PATHS["cache"]}/station_data'
            filepath = f"{savedir}/station_metadata_all.pkl"
            if os.path.exists(filepath):
                print(
                    f"Loading station metadata from cache: {filepath}, "
                    "set use_cache=False if you want to manually load them."
                )
                self.station_metadata_all = utils.open_pickle(filepath)
            else:
                self.station_metadata_all = self.process_stations.get_metadata_df(
                    self.var
                )
    ...
    def preprocess_topography(
        self, highres_coarsen_factor=30, lowres_coarsen_factor=10
    ):
        ...
        ds_elev_highres = self._get_highres_topography(
            self.ds_elev, highres_coarsen_factor
        )
    ...
    def preprocess_era5(self, coarsen_factor=10):
    ...
    def preprocess_stations(self, remove_stations=("None",)):
        ...
        self.station_metadata = self._filter_stations(
            self.station_metadata_all, remove_stations=remove_stations
        )
        self.station_raw_df = self._get_station_raw_df(self.station_metadata)

        return self.station_raw_df

    def _get_highres_topography(
        self,
        ds_elev,
        coarsen_factor=30,
        plot=False,
    ...
    def _compute_tpi(
        ...
        for window_size in [0.1, 0.05, 0.025]:
            ...
            smoothed_elev_da.data = gaussian_filter(
                smoothed_elev_da.data, sigma=scales, mode="constant", cval=0
            )
    ...
    def _filter_stations(self, df_station_metadata, remove_stations=[None]):
        ...
        df_filtered_years = df[
            (df["start_year"] < years[-1]) & (df["end_year"] >= years[0])
        ]

        if area is not None:
            df_filtered_area = df_filtered_years[
                (df_filtered_years["lon"] > PLOT_EXTENT[area]["minlon"])
                & (df_filtered_years["lon"] < PLOT_EXTENT[area]["maxlon"])
                & (df_filtered_years["lat"] > PLOT_EXTENT[area]["minlat"])
                & (df_filtered_years["lat"] < PLOT_EXTENT[area]["maxlat"])
            ]
        else:
            df_filtered_area = df_filtered_years
    ...
    def _get_station_raw_df(self, df_station_metadata):
        ...
        for path in tqdm(station_paths, desc="Loading filtered stations"):
            df = self.process_stations.load_station_df(
                path, var, daily=self.use_daily_data
            )
            df_list.append(df)
        # print('Concatenating station data...')
        df = pd.concat(df_list)
        station_raw_df = (
            df.reset_index().set_index(["time", "latitude", "longitude"]).sort_index()
        )

        ### filter years
        station_raw_df_ = station_raw_df.reset_index()
        station_raw_df_ = station_raw_df_[
            (station_raw_df_["time"] >= str(years[0]))
            & (station_raw_df_["time"] <= f"{str(years[-1])}-12-31")
        ]
        station_raw_df = station_raw_df_.set_index(
    ...
    def load_landmask(self):
        ...
        assert (
            self._ds_elev_hr is not None
        ), "_get_highres_topography() must be run first"
    ...
    def add_time_of_year(self, da):
        """
        Add cos_D and sin_D to output dataset to be used as context set,
        original da must have the time coordinate.
        Info: https://alan-turing-institute.github.io/deepsensor/user-guide/convnp.html
        """
    ...
    def process_all_for_training(
        ...
        data_processor = DataProcessor(
            ...
            x1_map=(
                highres_aux_raw_ds["latitude"].min(),
            ...
            x2_map=(
                highres_aux_raw_ds["longitude"].min(),
        ...
        aux_ds, highres_aux_ds = data_processor(
            [aux_raw_ds, highres_aux_raw_ds], method="min_max"
        )  # minmax
        landmask_ds = (
            data_processor(landmask_raw_ds, method="min_max")
    ...
    def test_normalisation(
        ...
        for ds, raw_ds, ds_name in zip(
            [era5_ds, aux_ds, highres_aux_ds],
    ...
    def plot_dataset(
        ...
        if type == "top_highres":
            ...
            raise ValueError(
                f"type={type} not recognised, choose from ['era5', 'top_highres', 'top_lowres']"
            )
        ...
        if with_stations:
            assert (
                self.station_metadata is not None
            ), "Run process_stations() first or set with_stations=False"
    ...
    def print_resolutions(self):
        ...
        print(
            "Topography highres:\n  "
            f"lon={resolutions['topography_high_res']['lon']:.4f}, "
            f"lat={resolutions['topography_high_res']['lat']:.4f}\n"
            "Topography lowres:\n  "
            f"lon={resolutions['topography_low_res']['lon']:.4f}, "
            f"lat={resolutions['topography_low_res']['lat']:.4f}\n"
            "ERA5:\n  "
            f"lon={resolutions['era5']['lon']:.4f}, "
            f"lat={resolutions['era5']['lat']:.4f} "
        )
        if (
            resolutions["topography_high_res"]["lon"] > resolutions["era5"]["lon"]
            or resolutions["topography_high_res"]["lat"] > resolutions["era5"]["lat"]
        ):
            warnings.warn(
                "highres topography resolution is higher than ERA5 resolution",
                UserWarning,
            )
        if (
            resolutions["topography_high_res"]["lon"]
            > resolutions["topography_low_res"]["lon"]
            or resolutions["topography_high_res"]["lat"]
            > resolutions["topography_low_res"]["lon"]
        ):
            warnings.warn(
                "lowres topography resolution is higher than highres topography resolution",
                UserWarning,
            )
