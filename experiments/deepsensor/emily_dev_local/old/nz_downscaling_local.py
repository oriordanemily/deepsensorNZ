...
print(
    f"Lowres lat resolution: {dataprocess.resolution(aux_raw_ds, 'latitude'):.4f} degrees"
)
print(
    f"Lowres lon resolution: {dataprocess.resolution(aux_raw_ds, 'longitude'):.4f} degrees"
)
print(
    f"Highres lat resolution: {dataprocess.resolution(highres_aux_raw_ds, 'latitude'):.4f} degrees"
)
print(
    f"Highres lon resolution: {dataprocess.resolution(highres_aux_raw_ds, 'longitude'):.4f} degrees"
)
print(
    f"Original lat resolution: {dataprocess.resolution(ds_elev, 'latitude'):.4f} degrees"
)
print(
    f"Original lon resolution: {dataprocess.resolution(ds_elev, 'longitude'):.4f} degrees"
)
...
era5_raw_ds = era_ds.sel(
    latitude=slice(top_max_lat, top_min_lat), longitude=slice(top_min_lon, top_max_lon)
)
...
aux_ds, hires_aux_ds = data_processor(
...
task_loader = TaskLoader(
    context=[era5_ds, aux_ds], target=station_df, aux_at_targets=hires_aux_ds
)
print(task_loader)
